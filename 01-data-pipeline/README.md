...
| 5 | **Data Validations (DCC)** | Validates referential integrity between staging tables. Checks that records link together correctly (transactions reference valid entities, headers have line items, line items have headers). Flags orphan records and duplicates. | Validated records |
| 6 | **Data Mapping** | Consolidates multiple SAP source files into unified business objects. Example: EKKO, EKPO, and LFA1 tables are combined into a single PurchaseOrders object with vendor details. Creates one clean record per transaction. | Unified business objects |
| 7 | **SQL Test Execution** | 100+ SQL stored procedures run fraud and risk detection tests per module. Test categories include Fraud Monitoring, High Risk Analysis, Conflict of Interest, Anti-Corruption, Statistical Testing, Internal Controls, and Revenue Leakage. Each module (P2P, O2C, T&E) has its own test library. | Flagged transactions (test hits) |
